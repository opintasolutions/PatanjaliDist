...
contactSubmit.addEventListener('click', () => {
  ...
  // condition
  if (name.length && email.length && number.toString().length && mssg.length) {
    const textBody = `Name:\n${name}\n\nContact:\n${number}\n\nEmail:\n${email}\n\nMessage:\n\n${mssg}`;
    ...
    rsvpBody.append(
      'content',
      `<h2>Hello ${name}</h2><br /><h3>Thank you for contacting Patanjali Distributors, your feedbacks are appreciated.</h3>`,
    );
    ...
    alert('Please fill out all the sections');
  }
});
